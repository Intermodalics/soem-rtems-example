...
#include <bsp/nexus-devices.h>  // Required for network interface
...
static int si_map_sdo(int slave)
{
    int wkc, rdl;
    int retVal = 0;
    uint8 nSM, iSM, tSM;
...
static int si_map_sii(int slave)
{
    int retVal = 0;
    int Tsize, outputs_bo, inputs_bo;
...
void slaveinfo(char *ifname, boolean print_sdo, boolean print_map)
{
   int cnt, i, j, nSM;
    uint16 ssigen;
    int expectedWKC;
   ...
   if (ec_init(ifname))
   {
      ...
      if ( ec_config(FALSE, &IOmap) > 0 )
      {
         ...
         for( cnt = 1 ; cnt <= ec_slavecount ; cnt++)
         {
            ...
            if ((ec_slave[cnt].mbx_proto & ECT_MBXPROT_COE) && print_sdo)
                    si_sdo(cnt);
                if(print_map)
            {
...
#define ARGC(a) (sizeof(a)/sizeof(*(a)) - 1)
void initialize_network() {
  assert(rtems_bsd_initialize() == RTEMS_SUCCESSFUL);
  ...
  assert(rtems_bsd_command_ifconfig(ARGC(ifconfig), ifconfig) == 0);
  sleep(1);
  ifconfig[2] = NULL;
  assert(rtems_bsd_command_ifconfig(2, ifconfig) == 0);
}
...
rtems_task Init(rtems_task_argument __unused ignored) {
